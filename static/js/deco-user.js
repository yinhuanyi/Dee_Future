    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status == "success"){
                Dml.fun.showErrorTips($('#jsChangePhoneTips'), data.msg);
        ...
        error: function(data){
            Dml.fun.showValidateError($('#jsChangeEmail'), '请求发送失败');
        },
...
// todo: 点击修改密码按钮，修改用户密码
function changeUserPwd($btn){
        $.ajax({
            cache: false,
            type: "POST",
            dataType:'json',
            url:"/users/update/pwd/", // 这里是js，因此不能使用模板语言
            data:$('#jsResetPwdForm').serialize(),
            async: true,
            success: function(data) {
                // 如果返回的信息中有password1这个属性，说明是第一个密码错误
                if(data.password1){
                    Dml.fun.showValidateError($("#pwd"), data.msg);
                }else if(data.password2){ // 如果返回的信息中有password2这个属性，说明是第二个密码错误
                    Dml.fun.showValidateError($("#repwd"), data.msg);
                }else if(data.status == "success"){ // 如果是返回的status为success，那么密码成功
